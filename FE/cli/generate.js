...
generateTemplateFiles([
  {
      ...
      },
      {
        ...
        newSlot: ({ __schemaTable__ }) => {
          ...
          return columnSearchAble
            ?.map((e) => {
              const mappingInput = {
                text: `<Input placeholder="Nhập ${e?.title?.toLowerCase()}" />`,
                select: `
                <Select allowClear placeholder="Nhập ${e?.title?.toLowerCase()}">
                  {[].map((e) => {
                    return (
              ...
              return mappingInput?.[e?.searchType]
                ? `<Form.Item label="${e?.title}" name="${e?.key}">
    ...
    output: {
      path: "./src/modules/manager/__name__(lowerCase)s",
